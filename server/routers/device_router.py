...
async def _verify_gateway_credential(sn: str, password: str) -> bool:
    try:
        ...
        raise HTTPException(status_code=503, detail="MQTT认证服务不可用") from exc
...
@router.get("")
async def list_devices(current_user: User = Depends(get_required_user), db: AsyncSession = Depends(get_db)):
...
@router.post("/bind")
async def bind_device(body: BindDeviceRequest, current_user: User = Depends(get_required_user), db: AsyncSession = Depends(get_db)):
    sn = body.sn.strip()

    existing = await db.scalar(select(MqttDeviceBinding).where(MqttDeviceBinding.sn == sn))
    if existing:
    ...
    return {"message": "绑定成功", "device": binding.to_dict()}


@router.delete("/{sn}")
async def unbind_device(sn: str, current_user: User = Depends(get_required_user), db: AsyncSession = Depends(get_db)):
