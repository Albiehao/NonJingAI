...
// Your trusted gateway. Do NOT put a ChatGPT/OpenAI token on ESP8266.
// Expected response example:
// {"codex_available":true,"five_hour_remaining":75,"weekly_remaining":82,"five_hour_reset_seconds":3214}
const char* STATUS_URL = "http://192.168.1.100:8080/codex/status";

// Optional shared secret between ESP8266 and your gateway.
...
// SSD1306 128x64 I2C. NodeMCU defaults: SDA=D2(GPIO4), SCL=D1(GPIO5).
...
String formatDuration(uint32_t seconds) {
  ...
  if (hours > 0) {
    ...
    snprintf(buf, sizeof(buf), "%02lu:%02lu", (unsigned long)minutes,
             (unsigned long)secs);
  }
  return String(buf);
}
